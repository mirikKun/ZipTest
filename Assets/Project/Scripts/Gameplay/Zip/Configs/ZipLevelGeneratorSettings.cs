...
{
    [CreateAssetMenu(fileName = "ZipLevelGeneratorSettings", menuName = "Configs/Zip/ZipLevelGeneratorSettings",
        order = 1)]
    public class ZipLevelGeneratorSettings : ScriptableObject
    {
        [SerializeField]
        private ZipLevelGenerator.GenerationSettings _generationSettings = new ZipLevelGenerator.GenerationSettings();
        ...
        public ZipBoardData GenerateLevel()
        {
            ...
            return new ZipBoardData(levelData.Size, levelData.CheckpointPositions, levelData.Walls,
                levelData.OrientedTimeToFinish);
        }
    }
}